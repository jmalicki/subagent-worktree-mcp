        ...
        for (name, description) in tools {
            ...
            match name {
                "spawn_subagent" => {
                    ...
                    doc.push_str(
                        "- `prompt` (string, required): Initial prompt to give to the subagent\n",
                    );
                    ...
                    doc.push_str(
                        "- `agent_options` (object, optional): Additional options for the agent\n",
                    );
                }
                "cleanup_worktree" => {
                    doc.push_str("**Parameters:**\n");
                    doc.push_str(
                        "- `worktree_path` (string, required): Path to the worktree to clean up\n",
                    );
