...
use subagent_worktree_mcp::subagent_spawner::{AgentOptions, SubagentSpawner};
...
async fn test_git_worktree_manager_creation() -> Result<()> {
    ...
    assert!(
        manager.is_ok(),
        "Failed to create GitWorktreeManager for valid git repo"
    );
    ...
    assert!(
        manager.is_err(),
        "Should fail to create GitWorktreeManager for non-git directory"
    );
...
async fn test_is_git_repo() -> Result<()> {
    ...
    assert!(
        manager.is_git_repo(),
        "Should identify git repository correctly"
    );
...
async fn test_create_worktree_basic() -> Result<()> {
    ...
    assert!(
        output.status.success(),
        "Git status should work in worktree"
    );
...
async fn test_create_worktree_with_base_branch() -> Result<()> {
    ...
    let worktree_path = manager
        .create_worktree("test-branch", Some("base-branch"), None)
        .await?;
    ...
    assert!(
        worktree_path.join("base-file.txt").exists(),
        "Worktree should contain base-file.txt"
    );
...
async fn test_create_worktree_custom_directory() -> Result<()> {
    ...
    let worktree_path = manager
        .create_worktree("test-branch", None, Some("custom-worktree"))
        .await?;
    ...
    assert!(
        ...
        "Custom worktree directory should exist"
    );
...
async fn test_create_worktree_existing_branch() -> Result<()> {
    ...
    let worktree_path = manager
        .create_worktree("existing-branch", None, None)
        .await?;

    // Should succeed and create worktree
    assert!(
        worktree_path.exists(),
        "Worktree should be created for existing branch"
    );
...
async fn test_list_worktrees() -> Result<()> {
    ...
    assert_eq!(
        worktrees.len(),
        1,
        "Should have one worktree initially (main)"
    );
    ...
    assert_eq!(
        worktrees.len(),
        3,
        "Should have 3 worktrees after creating 2 additional ones"
    );

    // Verify worktree information
    let branch_names: Vec<&str> = worktrees
        .iter()
...
async fn test_cursor_cli_options_custom() -> Result<()> {
    ...
    assert!(
        !options.new_window,
        "Custom option should not open new window"
    );
...
async fn test_integration_worktree_and_spawn() -> Result<()> {
    ...
    // Create worktree
    let worktree_path = manager
        .create_worktree("integration-test", None, None)
        .await?;
    ...
    assert!(
        worktree_path.join("README.md").exists(),
        "Worktree should contain expected files"
    );

    // Test that we could spawn cursor-agent (but don't actually do it to avoid side effects)
    let cursor_info = spawner.list_available_agents().await;
...
async fn test_error_handling_invalid_branch() -> Result<()> {
    ...
    let result = manager
        .create_worktree("test-branch", Some("non-existent-branch"), None)
        .await;
...
async fn test_cleanup_worktree_nonexistent() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Should fail when trying to clean up non-existent worktree"
    );
