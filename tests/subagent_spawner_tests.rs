...
async fn test_subagent_spawner_spawn_nonexistent_agent() -> Result<()> {
    ...
    let result = spawner
        .spawn_agent("non-existent-agent", &temp_path, "test prompt", &options)
        .await;

    // Should fail gracefully
    assert!(
        result.is_err(),
        "Should fail when spawning non-existent agent type"
    );
...
async fn test_subagent_spawner_spawn_with_invalid_path() -> Result<()> {
    ...
    let result = spawner
        .spawn_agent("cursor-agent", &invalid_path, "test prompt", &options)
        .await;
...
async fn test_subagent_spawner_spawn_with_empty_prompt() -> Result<()> {
    ...
    let result = spawner
        .spawn_agent("cursor-agent", &temp_path, "", &options)
        .await;

    // We don't assert success here since cursor-agent might not be available
...
async fn test_cursor_cli_agent_name_consistency() -> Result<()> {
    ...
    let agent1 = CursorCliAgent;
    let agent2 = CursorCliAgent;
...
async fn test_agent_options_edge_cases() -> Result<()> {
    ...
    assert_eq!(options.new_window, true, "Should handle new_window setting");
    assert_eq!(options.wait, true, "Should handle wait setting");
    ...
    assert_eq!(
        options_large.new_window, false,
        "Should handle new_window setting"
    );
    assert_eq!(options_large.wait, false, "Should handle wait setting");
    assert_eq!(options_large.detach, true, "Should handle detach setting");
...
async fn test_agent_options_custom_options() -> Result<()> {
    ...
    assert_eq!(
        ...
        2,
        "Should have two custom options"
    );
    assert_eq!(
        options.custom_options.get("test_key"),
        Some(&"test_value".to_string())
    );
    assert_eq!(
        options.custom_options.get("another_key"),
...
async fn test_agent_info_creation_edge_cases() -> Result<()> {
    ...
    assert_eq!(
        info_empty.available, false,
        "Should handle unavailable agent"
    );
    assert_eq!(info_empty.version, "", "Should handle empty version");
    assert_eq!(
    ...
    assert_eq!(info_long.available, true, "Should handle available agent");
    assert_eq!(info_long.version.len(), 1000, "Should handle long version");
    assert_eq!(
        info_long.description.len(),
        1000,
...
async fn test_subagent_spawner_concurrent_registration() -> Result<()> {
    ...
    assert_eq!(
        agents.len(),
        3,
        "Should handle multiple registrations correctly"
    );
...
async fn test_agent_spawner_trait_consistency() -> Result<()> {
    ...
    // Verify consistency
    assert_eq!(name, "cursor-agent", "Name should be consistent");
    assert!(true, "Availability check should not panic");
    assert_eq!(
        info.description, "Cursor CLI - AI-powered code editor",
        "Info description should be correct"
    );
