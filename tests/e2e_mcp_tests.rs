...
async fn test_e2e_complete_workflow() -> Result<()> {
    ...
    let list_response = client.send_request("tools/call", json!({
        "name": "list_worktrees",
        "arguments": {
            "include_agents": true,
    ...
    println!("Initial worktrees: {}", list_response["result"]["content"][0]["text"]);
    
    // Step 2: Spawn a subagent
    let spawn_response = client.send_request("tools/call", json!({
        "name": "spawn_subagent",
        "arguments": {
    ...
    println!("Spawn response: {}", spawn_response["result"]["content"][0]["text"]);
    
    // Step 3: Wait a moment for the agent to start
    sleep(Duration::from_millis(2000)).await;
    
    // Step 4: List worktrees again (should show our new worktree)
    let list_response2 = client.send_request("tools/call", json!({
        "name": "list_worktrees",
        "arguments": {
            "include_agents": true,
            "only_our_agents": true,
    ...
    println!("Worktrees after spawn: {}", list_response2["result"]["content"][0]["text"]);
    
    // Step 5: Clean up the worktree
    let cleanup_response = client.send_request("tools/call", json!({
        "name": "cleanup_worktree",
        "arguments": {
    ...
    println!("Cleanup response: {}", cleanup_response["result"]["content"][0]["text"]);
    
    // Step 6: Verify cleanup by listing worktrees again
    let list_response3 = client.send_request("tools/call", json!({
        "name": "list_worktrees",
        "arguments": {
            "include_agents": true,
            "only_our_agents": true
    ...
    println!("Worktrees after cleanup: {}", list_response3["result"]["content"][0]["text"]);
...
async fn test_e2e_spawn_subagent_variations() -> Result<()> {
    ...
    let response1 = client.send_request("tools/call", json!({
        "name": "spawn_subagent",
        "arguments": {
    ...
    assert!(response1["result"]["content"][0]["text"].as_str().unwrap().contains("Successfully spawned"));
    
    // Test 2: Spawn with all parameters
    let response2 = client.send_request("tools/call", json!({
        "name": "spawn_subagent",
        "arguments": {
    ...
    assert!(response2["result"]["content"][0]["text"].as_str().unwrap().contains("Successfully spawned"));
    
    // Clean up both worktrees
    let _ = client.send_request("tools/call", json!({
        "name": "cleanup_worktree",
        "arguments": {
...
async fn test_e2e_agent_monitoring() -> Result<()> {
    ...
    assert!(spawn_response["result"]["content"][0]["text"].as_str().unwrap().contains("Successfully spawned"));
    
    // Wait for agent to start
    sleep(Duration::from_millis(1000)).await;
    
    // Check for waiting agents
    let waiting_response = client.send_request("tools/call", json!({
        "name": "list_worktrees",
        "arguments": {
            "include_agents": true,
            "only_our_agents": true,
    ...
    println!("Waiting agents: {}", waiting_response["result"]["content"][0]["text"]);
    
    // Check all agents
    let all_agents_response = client.send_request("tools/call", json!({
        "name": "list_worktrees",
        "arguments": {
            "include_agents": true,
            "only_our_agents": true,
    ...
    println!("All agents: {}", all_agents_response["result"]["content"][0]["text"]);
...
async fn test_e2e_error_handling() -> Result<()> {
    ...
    let invalid_tool_response = client.send_request("tools/call", json!({
        "name": "invalid_tool",
        "arguments": {}
    })).await;
    ...
    let missing_params_response = client.send_request("tools/call", json!({
        "name": "spawn_subagent",
        "arguments": {
    ...
    println!("Cleanup non-existent: {}", cleanup_response["result"]["content"][0]["text"]);
...
async fn test_e2e_concurrent_operations() -> Result<()> {
    ...
    let futures = (0..3).map(|i| {
        ...
        async move {
            client_ref.send_request("tools/call", json!({
                "name": "spawn_subagent",
                "arguments": {
    ...
    for (i, result) in results.iter().enumerate() {
        match result {
            Ok(response) => {
                println!("Concurrent spawn {} succeeded: {}", i, response["result"]["content"][0]["text"]);
            }
            Err(e) => {
    ...
    println!("Concurrent worktrees: {}", list_response["result"]["content"][0]["text"]);
...
async fn test_e2e_server_lifecycle() -> Result<()> {
    ...
    for i in 0..5 {
        let response = client.send_request("tools/call", json!({
            "name": "list_worktrees",
            "arguments": {}
        })).await?;
