...
async fn test_readme_documentation_matches_implementation() -> Result<()> {
    ...
    for tool in &documented_tools {
        assert!(
            ...
            "Documented tool '{}' is not implemented", tool
        );
    }
    ...
    for tool in &implemented_tools {
        assert!(
            ...
            "Implemented tool '{}' is not documented in README", tool
        );
    }
    
    println!("✅ All {} documented tools match implementation", documented_tools.len());
...
fn test_subagent_config_fields() {
    ...
    assert!(config.agent_options.is_some(), "agent_options should be optional");
}
...
fn test_agent_monitor_config_fields() {
    ...
    assert!(config.only_waiting_agents, "only_waiting_agents should be present");
    assert!(config.agent_types.is_some(), "agent_types should be optional");
    assert!(config.worktree_paths.is_some(), "worktree_paths should be optional");
}
...
fn test_spawn_subagent_schema() {
    ...
    assert!(!config.branch_name.is_empty(), "branch_name should be required");
...
fn test_monitor_agents_schema() {
    ...
    assert!(!config.only_our_agents, "only_our_agents should default to false");
    assert!(!config.only_waiting_agents, "only_waiting_agents should default to false");
    assert!(config.agent_types.is_none(), "agent_types should be optional");
    assert!(config.worktree_paths.is_none(), "worktree_paths should be optional");
}
...
fn test_list_worktrees_schema() {
    ...
    assert!(true, "list_worktrees should take no parameters as documented");
}
