...
async fn test_agent_monitor_config_default() -> Result<()> {
    ...
    assert_eq!(
        config.only_our_agents, false,
        "Default only_our_agents should be false"
    );
    assert_eq!(
        config.only_waiting_agents, false,
        "Default only_waiting_agents should be false"
    );
    assert_eq!(
        config.agent_types, None,
        "Default agent_types should be None"
    );
    assert_eq!(
        config.worktree_paths, None,
        "Default worktree_paths should be None"
    );
...
async fn test_agent_monitor_config_custom() -> Result<()> {
    ...
    assert_eq!(
        config.only_our_agents, true,
        "Custom only_our_agents should be true"
    );
    assert_eq!(
        config.only_waiting_agents, true,
        "Custom only_waiting_agents should be true"
    );
    assert_eq!(
        config.agent_types,
        Some(vec!["cursor-cli".to_string()]),
        "Custom agent_types should be set"
    );
    assert_eq!(
        config.worktree_paths,
        Some(vec!["/tmp/test".to_string()]),
        "Custom worktree_paths should be set"
    );
...
async fn test_agent_process_info_creation() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Waiting for input should be false"
    );
    assert_eq!(info.spawned_by_us, true, "Spawned by us should be true");
    assert_eq!(
        info.worktree_path,
        Some("/tmp/test-worktree".into()),
        "Worktree path should be correct"
    );
...
async fn test_agent_summary_creation() -> Result<()> {
    ...
    assert_eq!(
        summary.waiting_for_input, 2,
        "Waiting agents should be correct"
    );
    assert_eq!(summary.spawned_by_us, 3, "Our agents should be correct");
    assert_eq!(
        summary.total_cpu_usage, 10.5,
        "Total CPU usage should be correct"
    );
    assert_eq!(
        summary.total_memory_usage, 1024000,
        "Total memory usage should be correct"
    );
...
async fn test_agent_monitor_filtering_by_worktree_paths() -> Result<()> {
    ...
    let config = AgentMonitorConfig {
        ...
        only_waiting_agents: false,
        agent_types: None,
        worktree_paths: Some(vec![
            "/tmp/worktree1".to_string(),
            "/tmp/worktree2".to_string(),
        ]),
    };
